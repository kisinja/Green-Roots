...
export default async function OrdersPage() {
  ...
  const orders = await prisma.order.findMany({
    ...
    // ✅ receiptUrl is a scalar column on Order, not a relation —
    // it's returned automatically with findMany, no `include` needed.
    // `include` only accepts relation fields (like `items` below).
    include: {
          ...
          {orders.map((order) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span
                  className={`text-xs font-bold px-3 py-1 rounded-full ${STATUS_COLORS[order.status] || "bg-gray-100 text-gray-600"}`}
                >
                  {order.status}
                </span>
              </div>
